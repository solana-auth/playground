...
console.log(pico.green(`Solana Auth - Sign Transaction`))

const method: SolanaAuthMethod = 'solana:signTransaction'
...
api.log(`STEP 2. [API] Creates a transaction`)
...
alice.log(`STEP 3. [WEB] Alice signs the transaction`)
const signed = await alice.signTransaction(message)
alice.log(pico.gray(`Signed: ${JSON.stringify(signed, null, 2)}`))

// STEP 4 - Verify the signed message
api.log(`STEP 4. [API] API verifies the signed transaction`)
