...
console.log(pico.green(`Solana Auth - Sign In`))

const method: SolanaAuthMethod = 'solana:signIn'
...
api.log(`STEP 2. [API] Creates a sign in message`)
...
alice.log(`STEP 3. [WEB] Alice signs the sign in message`)
const signed = await alice.signIn(message)
alice.log(pico.gray(`Signed: ${JSON.stringify(signed, null, 2)}`))

// STEP 4 - Verify the signed message
api.log(`STEP 4. [API] API verifies the signed sign in message`)
